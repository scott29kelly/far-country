...
2. **M3.2 — City shell.** *(done, re-platformed — updated 2026-07-13)* The base tier is a real jasper wall built from `cityModel.ts`'s shared tables, with genuine gaps at the twelve named gates in Ezekiel 48:30–34 order and a twelve-stone faceted foundation course notched at every gate threshold. PR #26 added lateral collision derived from those same tables: walk and fly stop at the wall, foundation course, plinth, and terrace massing; gate gaps remain real passages; oblique input slides along faces. The implementation, live ground-probe regressions, wall stop, gate passage, slide behavior, and foundation notches were verified on real Windows/Intel Xe-LPG hardware before merge. Gate tribe labels remain a wayfinding/HUD concern under M3.4, not missing shell geometry.
3. **M3.3 — Step-mountain, throne, river, tree of life.** *(largely done, re-platformed — updated 2026-07-02)* The M3 material/geometry pass replaced the box massing with translucent gold-glass tiers over glowing interiors, instanced architectural relief (arch frames, fluted piers, ivory cornice/arcade courses), faceted foundation gems, and iridescent pearl gates; the river is real crystal water (refraction/SSR/foam, ribbon cascades, caustic gold beds, walk-guard); the trees of life are real pipeline-built hero trees with wind, GI and glowing fruit; and the summit now carries RENDERING-DECISIONS #4's rainbow halo + sea of glass (no longer `(legacy only)`). The throne remains conflated with the glory-light as one aniconic primitive (ADR 0010). Still open here: mini-map, click-to-teleport (`(legacy only)`). City renders at **citywide scale (~2.5 mi, `NJ_SCALE=20`)** per [ADR 0014](adr/0014-citywide-scale-rendering.md), which supersedes this milestone's original ~200 m-placeholder framing entirely. Inventory + verification: `apps/world-engine/STATUS.md` "New Jerusalem scene" 2026-07-02 entry.
4. **M3.4 — Entity interaction.** *(not ported)* No HUD, no descriptor cards, no click-picking of any kind exists in `/world-preview` — the engine has only its own generic debug HUD (fps chip, `F3` panel). Everything in this milestone is `(legacy only)`.
5. **M3.5 — Symbolic indicators.** *(not ported)* No tier badges, no literal-vs-symbolic visual key. `(legacy only)`.
6. **M3.6 — Population.** *(not started on either scene, unchanged.)* Still policy-ready (ADR 0011) but unbuilt.
...
**Done when:** A user can walk through a recognizable representation of the New Jerusalem on a mid-range laptop, click any major feature, and see a sourced descriptor for it. **Not yet met by `/world-preview`** — see Task 1–3 work tracked in this session.
