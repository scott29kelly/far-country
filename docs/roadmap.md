...
> **Population fidelity note (M3.6) — raised 2026-07-31; re-scoped 2026-08-01 by [ADR 0019](adr/0019-photorealistic-redeemed-humans.md).** The great multitude currently renders as inverted cones with sphere heads — placeholder geometry. This is owed M3.6 polish and is Phase 3 work, not Phase 4. **Target (ADR 0019, Scott's call on Rev 7:13–14 — the multitude is explicitly redeemed *humans*): photorealistic human beings** — real bodies, faces, skin, hair, white robes and palm branches (Rev 7:9), visibly diverse in ethnicity, age and build. Guards that remain: every face is synthetic and anonymous (no real-person likenesses, no named-biblical-person portraits — ADR 0019 rule 2); divine persons stay absolutely aniconic (ADR 0010); angels keep their settled light-pillar rendering (ADR 0019 rule 5). Engineering note: ~12,700 instances make this a crowd-rendering + level-of-detail problem — `apps/world-engine/src/vegetation/Impostors.ts` already implements the atlas-impostor pattern needed at distance; near-camera figures need real human meshes and animation. Requires GPU review at every step; CPU probes cannot judge it.
