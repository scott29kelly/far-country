...
6. **M3.6 — Population.** *(first pass built on `/world-preview`, 2026-07-18)* The settled RENDERING-DECISIONS #3 rendering is live under ADR 0011/0010: ~12,700 white-robed faceless figures with raised palm branches (Rev 7:9, `great-multitude`) in forty worship assemblies on the plaza ring and terrace pavements, all facing the summit light, plus forty-eight abstract light-pillar hosts (Rev 5:11, `myriads-of-angels`) ringing the summit in twelve slowly rising-and-falling clusters — both clickable/walk-discoverable as zone-level cited entities (`src/nj/populationModel.ts`, `Population.ts`; 13-case CPU probe). The four living creatures and twenty-four elders stay omitted per ADR 0011 rule 4. Figure idle motion landed 2026-07-21 (shader-time sway, feet planted, heads riding the robe crown — `apps/world-engine/STATUS.md` later-3 entry) and gained worship-idle head articulation 2026-08-10 (procedural gaze drift + nods about the neck pivot, per-figure phase, shader-analytic — Scott's no-downloads call; STATUS 2026-08-10 later entry); still open: M4.4's nations/pilgrimage dynamism and Scott's subjective pass. Correction (2026-07-21): the sixteen plaza-ring assemblies had been stationed inside the wall's solid massing — entombed, invisible, unreachable; the wall-slab/gallery fix (RENDERING-DECISIONS entry #10) recentres them into the covered street-of-gold gallery, walkably reachable through the gates for the first time. Inventory: `apps/world-engine/STATUS.md` 2026-07-18 later-5 entry + 2026-07-21 entry.

> **Population fidelity note (M3.6) — raised 2026-07-31; re-scoped 2026-08-01 by [ADR 0019](adr/0019-photorealistic-redeemed-humans.md); rebuild steps 1-2 built 2026-08-03.** The cone-and-sphere placeholders are gone: the multitude now renders as **seeded parametric human figures under a GPU crowd LOD** (`apps/world-engine/src/nj/Crowd.ts` compute-cull → indirect draws; `figureModel.ts` + `FigureMesh.ts` — six archetypes spanning age and build, uniform skin/hair palette draws, white robes and palm branches kept, ~2M on-screen triangle budget probe-guarded, far ring as captured atlas impostors). Guards that remain: every face is synthetic and anonymous (no real-person likenesses, no named-biblical-person portraits — ADR 0019 rule 2); divine persons stay absolutely aniconic (ADR 0010); angels keep their settled light-pillar rendering (ADR 0019 rule 5). **Near-ring tier under way — [ADR 0020](adr/0020-vendored-generated-assets-anny-path.md) (the Anny path, slice 1 built 2026-08-04, slice 2 built 2026-08-05, slice 3 built 2026-08-10):** the near ring (≤30 m) now renders vendored offline-generated heads, eyes and hands from the Anny parametric human (CC0 artist-authored morph spaces — anonymity by construction; `pipeline/figures/` + `figuresVendored.gen.ts`), and slice 2 gives them real skin: per-vertex hm08 UVs carried through decimation plus a vendored 2×2 CC0 MakeHuman skin-diffuse atlas (pinned-commit, sha256-verified sources), sampled in the one crowd material keyed by each figure's `skin01` and normalized so the authored SKIN_RAMP keeps owning average tone. Real skull/ear/jaw geometry under real albedo at arm's length; slice 3 closes the open-hand simplification: the frond hand grips (authored finger curls over the model's own rig, fingers only — wrist and placement contract untouched) and the hanging hand relaxes, with the slice-1/2 constant wrist-anchor offset corrected in the same pass. The four CC0 skin-texture sources are now vendored in-repo (pipeline/figures/vendor/skins/) after the upstream MakeHuman assets repo vanished from GitHub — the supply-chain failure ADR 0020's posture anticipated. Scott's hair verdict (2026-08-12, vendored CC0 meshes) is executed: six MakeHuman system hair styles (obj+mhclo sources vendored at pipeline/figures/vendor/hair/), mhclo-seated on the default base mesh and affine-fitted per archetype, replace the procedural shells at LOD0 — real haircuts at arm's length, ramp-colored, ≤800 tris each, LOD1 keeps the shell (STATUS 2026-08-12 entry). The animation track opened 2026-08-10 with the procedural worship-idle head articulation (Scott's no-downloads call); full-body baked-animation crowds stay reserved for M4.4. Still open: M4.4's dynamism and Scott's subjective pass. GPU review at every step; CPU probes guard placement, budgets, the bloom contract, and the vendored module's provenance.
...
**Done when:** A user can walk through a recognizable representation of the New Jerusalem on a mid-range laptop, click any major feature, and see a sourced descriptor for it. **Substantially met by `/world-preview` as of 2026-07-18** — every major grounded feature (city, wall, gates, foundations, street, river, trees of life, throne/glory, sea of glass, temple zone) is clickable with a sourced descriptor card; the dwelling campus picks as its cited Ezek 45:4-5 zone entities as of 2026-07-21, and Scott's subjective acceptance pass is still owed.
...
4. **M4.4 — Mortal nations + pilgrimage dynamism.** *(scoped 2026-08-13; increment 1 built)* The millennial earth is inhabited by mortal nations alongside resurrected saints; the city's perpetually-open gates (Rev 21:25) take on their premillennial meaning — nations and kings bring their glory in (Rev 21:24–26) and make pilgrimage to worship (Zech 14:16). A representative, non-static depiction of that movement. Figural non-divine persons follow [`adr/0011-population-rendering-policy.md`](adr/0011-population-rendering-policy.md); divine persons remain aniconic (ADR 0010). **Increment 1 (2026-08-13, Scott's scope call — worship motion first): the multitude's worship cycles are live** — per-figure bow/kneel/arm-lift cycles, shader-analytic channel curves on the engine's freezable clock, zero CPU per-instance work (`apps/world-engine/STATUS.md` 2026-08-13 entry). Remaining: pilgrimage locomotion (nations moving through the gates — the baked-skeletal-motion path; source decided 2026-08-13: Quaternius CC0 walk clips banked in `pipeline/figures/.cache/`, NVIDIA ARDy text-to-motion approved for authoring, retarget + bake pipeline to build) and Scott's subjective pass on the whole dynamism layer.

> **Layout note (city and neighbourhoods on the Holy Allotment) — raised 2026-07-31, owed by M4.2.** The rendered relationship between the New Jerusalem city and the raised Holy-Allotment platform does not match the reference layout: the city should sit **nearer one edge** of the platform, and the platform should carry **more open ground relative to the city's footprint** than it currently does. The Zadok (priests') and Levite neighbourhood bands must also be checked for correct position relative to each other, to the temple, and to the surrounding landscape. This is a *placement/scale* correction across `apps/world-engine/src/nj/rimModel.ts` (platform), `cityModel.ts` (city footprint) and `campusModel.ts` (bands) — note the band extents are already resolver-driven from the cited Ezek 45/48 records, so any change must move the **platform and city** relative to those bands, not re-hand-type the cited numbers. Verify against the reference art before adjusting; needs GPU review (`tools/shoot.ts`), not CPU probes.

> **Measurement reference.** A hand-authored dimensions chart is deliberately NOT the answer here: the cited measurements already live as generated data (`templeMeasurements.gen.ts` / `EZT`, `allotmentMeasurements.gen.ts` / `EZA`, realized at the literal long cubit by ADR 0018's resolver), and a parallel hand-typed table would be exactly the desync the "no hand-mirrored constants" discipline forbids. If a blueprint-style overview is wanted, **generate** it from those same records so it cannot drift.
