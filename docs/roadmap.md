...
> **Population fidelity note (M3.6) — raised 2026-07-31, Scott's subjective pass.** The great multitude currently renders as inverted cones with sphere heads, which reads as placeholder geometry beside the engine's terrain/vegetation quality. This is the owed M3.6 polish and is Phase 3 work, not Phase 4. **Constraint: [ADR 0011](adr/0011-population-rendering-policy.md) rule 3 governs and is not up for revision here** — non-divine figures stay *stylised, generic, non-individualised*; no faces, no portraiture, no individual likeness. That rule bounds *individuation*, not *craft*: robe silhouette, cloth behaviour, proportion, material response, contact shadowing and crowd pose variation can all rise a long way while the figures remain faceless and generic. The upgrade is "a convincing anonymous robed crowd", never "photoreal people". Engineering note: ~12,700 instances make this an LOD problem — `apps/world-engine/src/vegetation/Impostors.ts` already implements the atlas-impostor pattern this needs at distance. Requires GPU review; CPU probes cannot judge it.
